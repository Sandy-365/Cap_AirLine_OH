...
public class CheckInServiceImpl : ICheckInService
{
    ...
    public async Task<CheckInDto> OnlineCheckInAsync(OnlineCheckInDto dto, string passengerName, string flightNumber, int flightId, DateTime departureTime, decimal fare, string token)
    {
        ...
        // Notify other services - Updated to match 7-argument constructor

        // 1.5 Trigger Reward Points Credit (Phase 5)
    ...
    public async Task<CheckInDto> StaffCheckInAsync(StaffCheckInDto dto)
    {
        ...
        // 1. Mark as CheckedIn in BookingService (Updated to match 7-argument constructor)

        // 2. Trigger Reward Points Credit (Phase 5)

        _logger.LogInformation("Staff Check-in completed for Passenger {PassengerId}. Reward event published.", dto.PassengerId);
        return MapToDto(checkIn);
    }
